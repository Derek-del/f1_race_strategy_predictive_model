...
<body class="route-overview">
    ...
    <header class="hero">
      <div class="hero-inner">
        <div>
          ...
          <p class="hero-note">Strategy prediction model for Red Bull Racing and Max Verstappen for the 2025 Formula 1 season</p>
  ...
  <script>
    ...
    function displayDriver(driverCode) {
      const code = safeText(driverCode).toUpperCase();
      return DRIVER_LABELS[code] || safeText(driverCode);
    }

    function displayTeam(teamName) {
      const key = safeText(teamName).toUpperCase();
      if (key.includes('RED BULL')) return 'Red Bull Racing';
      return safeText(teamName);
    }
    ...
    function openDrawer(row) {
      ...
      body.innerHTML = `
        <div>
          ...
          <span class='tag'>${displayTeam(row.team)}</span>
          <span class='tag'>${displayDriver(row.driver)}</span>
    ...
    function renderHeader(payload) {
      ...
      const team = rows.length ? displayTeam(rows[0].team) : "Red Bull Racing";
      const driver = rows.length ? displayDriver(rows[0].driver) : "Max Verstappen";
      document.getElementById('metaLine').textContent = rows.length
        ? `${rows.length} race strategies loaded for the ${firstYear} season. Model target: ${team} · ${driver}.`
        : `Race strategies ready for the ${firstYear} season. Model target: ${team} · ${driver}.`;
    }
